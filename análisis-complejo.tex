...
\end{defi}
 ...
 \textbf{Forma trigonométrica}: $z = a+bi = \abs{z}(\cos \alpha + i\sin \alpha)$
 
 \textbf{Forma exponencial}: $z = a+bi = \abs{z}e^{i\alpha}$
$$ 
...
Sea $z\neq 0$, entonces el logaritmo principal o rama principal del logaritmo de $z$, denotado por $\Log$, viene definido por $\Log z = \ln(\abs{z}) + i \Arg(z)$ donde $\Arg(z)\in (-\pi,\pi]$.Denotamos por $\log(z)$ al conjunto de elementos $w \in \C$ de forma que $e^w = z$. 
\end{defi}

\begin{defi}
Sea $\theta_0 \in\R$ Definimos $\log_{\theta_0} z = \ln \abs{z} + i\theta$ con \\$\theta \in (\theta_0,\theta_0+2\pi]$.
...
\textbf{Nota:} Si $alpha \in\R$ entonces $z^\alpha = \abs{z}^\alpha e^{i\alpha\arg z}$.
\end{defi}
...
Con $M_f(\gamma) = \max \{\abs{f(z)},z \in \gamma^* \}$ y $L(\gamma)$ la longitud de $\gamma$.
\end{prop}
...
\begin{multline*}\forall z\in U, \ g(z) := f(z) -f(z_0) -(z-z_0)f'(z_0),\ \forall \varepsilon>0 \Rightarrow\\
 \abs{g(z)}\leq \varepsilon|z-z_0 \text{ si } |z-z_0|< \delta|
 \end{multline*}
 \begin{align*}
...
$$f^{(n)}(z)n(\gamma,z) = \frac{n!}{2\pi i}\int_\gamma \frac{f(w)}{(z-z_0)^{n+1}}dw$$
\end{theorem}
...
\begin{dem}
Si $f(z)$ es acotada entonces $\exists M>0 / \abs{f(z)}<M \ \forall z\in \C$. Por la estimación de Cauchy para $n=1$:
...
Sabemos que $\displaystyle \abs{P(z)}\xrightarrow[\abs{z}\to 0]{} 0 \Rightarrow \exists R>0 / \abs{P(z)}>1 \text{ si } \abs{z} >R \Rightarrow \abs{f(z)} = \frac{1}{\abs{P(z)}} <1$ si $\abs{z}\leq R$. 
En consecuencia $f(z)$ es acotada en $\C$ y por el Teorema de Liouville $f(z) = cte.\ \sharp$
\end{dem}
...
Si $f(z)$ es analítica en un abierto $U$ y supongamos que $\abs{f(z)}$ tiene máximo en $z_0\in U$. Entonces $f(z)$ es constante en un entorno de $z_0$.
\end{theorem}

\begin{dem}
Por hipótesis sabemos que $\exists D(z_0,r),\ r>0$ tal que $\abs{f(z)} \leq \abs{f(z_0)}\ \forall z \in D(z_0,r)$ Entonces se cumple que
$$
\abs{f(z_0)} = \frac{1}{2\pi}\abs{\int_0^{2\pi}f(z_0+re^{it})dt} \leq \frac{1}{2\pi}\max\{|f(z_0+re^{it})/t\in [0,2\pi]\}\leq \abs{f(z_0)}
$$
$$
\Rightarrow \abs{f(z_0)} = \frac{1}{2\pi}\int_0^{2\pi}\abs{f(z_0+re^{it})}dt \Rightarrow \int_0^{2\pi} \abs{f(z-0)}-\abs{f(z_0+re^{it})}dt = 0
$$
$$
\Rightarrow \abs{f(z_0)} = \abs{f(z_0+re^{it})}\ \forall t\in[0,2\pi]\Rightarrow \abs{f(z_0)}= cte. \text{ en un entorno de } z_0.
$$$\qed$
\end{dem}

\begin{col}
Sea $f(z_0)$ analítica en un abierto y conexo $U$, supongamos que $\abs{f(z_0)}$ alcanza el máximo absoluto en $U$, entonces $f(z_0)$ tiene valor constante en $U$.
\end{col}
\begin{dem}
Por hipótesis $\exists z_0 \in U / \abs{f(z)}\leq \abs{f(z_0)} \ \forall z \in U$.
...
Sea $z\in E \Rightarrow \abs{f(z)} = \abs{f(z_0)} = \max\{\abs{f(z)}\}$ entonces por el resultado anterior $f$ es constante en un entorno de $z$. Con lo que $\exists\  r>0$ tal que $D(z,r)\subset E \Rightarrow E$ es abierto y entonces $E=U$. $\qed$
\end{dem}

\begin{theorem}[Principio del módulo máximo]
Sea $f(z)$ analítica en un conjunto abierto, conexo y acotado $U$ y continua en $Fr(U)$. 

Si $M =\max \{\abs{f(z)}/ z\in Fr(U)\}$, se cumple que:

\begin{enumerate}
\item $\abs{f(z)}\leq M \ \forall z \in U$
\item Si $\abs{f(z)} = M$ para $z_0 \in U$ entonces $f(z) = cte.$ en $U$.
\end{enumerate}
\end{theorem}

\begin{dem}\ \\
\begin{enumerate}
\item Observamos que $\abs{f(z)}$ es también continua en $\bar{U}$ por lo que $\abs{f(z)}$ alcanza un máximo en $\bar{U}$. Si tal máximo se alcanza en $Fr(U)$ hemos terminado.
Si el máximo se alcanza en  $Int(U)$ entonces $\exists z_0 \in U$ tal que $\abs{f(z)}\leq \abs{f(z_0)} \ \forall z\in U$ entonces $f$ es constante en $U$.
\item Si $\exists z_0 \in U$ tal que $\abs{f(z_0)} = M$ entonces $f(z_0)$ es constante.$\qed$
\end{enumerate}
\end{dem}


\begin{theorem}[Principio del módulo mínimo]
Sea $f(z)$ analítica en un conjunto abierto, conexo y acotado $U$ y continua en $Fr(U)$.Supongamos $f(z)\neq 0\ \forall z\in U$. Si $m =\min \{\abs{f(z)}/ z\in Fr(U)\}$, se cumple que:
\begin{enumerate}
\item $\abs{f(z)}\geq n \ \forall z \in U$
\item Si $\abs{f(z)} = n$ para $z_0 \in U$ entonces $f(z) = cte.$ en $U$.
\end{enumerate}
\end{theorem}
...
\begin{dem}
Sea $g(z)= e^{f(z)}$ analítica en $U$ y continua en la frontera. $\abs{g(z)} = \abs{e^{f(z)}} = \abs{e^{\Re(f(z))}}\cdot \abs{e^{\Im{f(z)}i}} = \abs{e^u}\cdot\abs{e^{iv}} = e^u$ como $g$ satisface el principio del módulo máximo (mínimo) $u$ también los cumple.
...
Sea $f(z)$ una función analítica en $D(0,1)$. Supongamos que $f(0)=0$ y $\abs{f(z)}\leq 1\ \forall z \in D(0,1)$ entonces $\abs{f(z)} \leq \abs{z}$ y $|f'(0)| \leq 1$.

Además si $\exists z_0 \in D(0,1)\backslash \{0\}$ tal que $\abs{f(z_0)} = \abs{z_0}$ o que $|f'(z_0)| = 1$ entonces $f(z) = az$  $\forall z\in D(0,1)$ , $as \in \C/ \abs{a}=1$.
\end{lema}
...
Por el P.M.Máximo $\abs{g(z)}\leq M = \max\{\abs{g(z)}/ z\in FR(U)\}$, $\abs{g(z)} = \frac{\abs{f(z)}}{z}\leq 1 \Rightarrow \abs{f(z)} \leq z \ \forall z\in D(0,1)$.
...
\begin{defi}
Una sucesión de números complejos $\{z_n\}_{n\geq 1}$ es convergente a $z\in\C$ si $\forall\varepsilon>0 \ \exists n_0\in\N / \abs{z_n-z}<\varepsilon\ \text{ si } n>n_0$.
\end{defi}
...
$b)$ Converge absolutamente si $\sum_{n\geq 1} \abs{z_n} < \infty$.
\end{defi}
\textbf{Observación:}
\begin{enumerate}
...
Tomemos ahora $k\subset D(z_0,r)$ un conjunto compacto, y sea $z' \in D(z_0,r)$ tal que $k\subset \bar{D}(z_0,r')\subset D(z_0,r)$ con $r' = \abs{z'-z_0}$.
...
&=\lim_{N_2 \to \infty} \abs{ \sum_{j = N_1}^{N_2}a_j(w-z_0)^j  } \leq \lim_{N_2\to \infty} \sum_{j=N_1}^{N_2}|a_j||w-z_0|^j \leq\\
&\leq lim_{N_2 \to \infty} \sum_{j=N_1}^{N_2}|a_j|\abs{z'-z_0}^j \leq \sum_{n\geq 0}|a_n|\abs{z'-z_0}^n < \infty
\end{align*}
...
\item Por reducción al absurdo, tomemos que la serie converge para algún $z' / \abs{z'-z_0}>r$.

Por el teorema de Abel la serie también convergería en $z\in D(z_0,r')$ con $r = \abs{z'-z_0}$.

En particular converge $\forall z / \abs{z-z_0} <r'$, es decir, también converge en $z/ r<\abs{z-z_0\abs{z}'-z_0}$ lo que supone una contradicción ya que 
$$\limsup\abs{a_n(z-z_0)^n}^{\frac{1}{n}} = \frac{\abs{z-z_0}}{r}>1\ \sharp$$
\end{enumerate}
\end{dem}
...
\item $f(z)$ es indefinidamente derivable en  $D(z_0,r)$ y \\
$f^{(p)}(z) = \sum_{n\geq 0}a_n\ n(n-1)\ldots (n-p+1)\cdot(z-z_0)^{n-p},\ p\in \N$.
\end{enumerate}
\end{theorem}
...
\begin{defi}
DEcimos que $f(z)$ tiene una singularidad en $z_0$ si $f(z)$ no es analítica en $z_0$ pero sí lo es en algún punto de todo entorno de $z_0$.
...
Sea $f(z)$ analítica en la corona $U = \{z\in\C / S_1 \leq |z-z_0|\leq S_2\}$ con $0\leq S_1\leq S_2\leq \infty$ y algún $z_0 \in \C$. Para todo $z\in U$ se tiene que:
$$f(z) = \sum_{n=-\infty}^{\infty} an(z-z_0)^n,\ a_n = \frac{1}{2\pi i}\int_C \frac{f(w)}{(w-z)^{n+1}}dw$$
Con $C = (C(z_0,r)$ y $S_1 < r< S_2$.
\end{theorem}
...
&\Leftrightarrow g(z) = f(z)(z-z_0)^m = a_{-m} + a_{m-1}(z-z_0)+\ldots \Leftrightarrow\\
...
\begin{enumerate}
\item $f(z)$ tiene una singularidad evitable en $z_0$ si $a_n=0\ \forall n<0$.
...
2.$(\Leftarrow)$ Si $\lim_{z\to z_0}\abs{f(z)}= \infty \Rightarrow f(z)$ po está acotada en un entorno perforado de $z_0$, entonces es polo o singularidad esencial y por el Teorema de Casorati-Weierstrass $z_0$ no es singularidad esencial $\Rightarrow$ es polo.
...
\begin{defi}
Diremos que $f(z)$ tiene una singularidad en $\infty$ si $f(z)$ es analítica en $\{z\in\C / \abs{z}>r\}$ para algún $r>0$.
...
2. $\infty$ es un polo de $f\Leftrightarrow z=0$ es polo de $g(z) = f\left(\frac{1}{z}\right)$ entonces $\lim_{z\to 0}\abs{g(z)} = \infty$. Además
...
Sea $f$ analítica en el semicírculo $\Gamma_R = \{z/ Re^{i\theta},0\leq \theta\leq \pi\}$ y sea $M(R) = \max\{\abs{f(z)}/ z\in \Gamma_R\}$. Si $a>0 \Rightarrow$
...
\begin{prop}
Sea $f(z)$ una función analítica en $\{z/\abs{z}>r,\Im(z)\geq 0\}\forall r>0$ y consideremos el arco semicircular de centro $0$ y radio $R$ positivamente orientado dado por
...
\begin{dem}
Por la desigualdad supuesta observamos en primer lugar que $\gamma$ no pasa por ningún cero de $f$. Así podemos definir $h(z) = 1 +\frac{g(z)-f(z)}{f(z)}$ en un cierto entorno de $\gamma^*$.
...
Así por el apartado 3 de la proposición anterior existe un abierto $V\ni z_0$ tal que $\forall z\in V\backslash\{z_0\}$ existe un único $w \in V\backslash\{z_0\}$ de imagen $g(z)$ entonces $f$ es inyectiva en $V$.
...
\begin{col}
Una función $f(z)$ es meromorfa en $\C$ si, y sOlo si, $f(z) = \frac{g(z)}{h(z)}$ con $g(z)$ y $h(z)$ funciones enteras y siendo $h(z)$ no idénticamente nula.
\end{col}
